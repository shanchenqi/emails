...
def keywords_to_scv(file_name, stopwordsfile, date1="2008-1-1", date2="2020-12-12"):
    ...
    for name in names:
        if not name in dic and len(name)>0:
            ...
            keywords_tfief = getkeywords_tfidf_sklearn(text)
            # print("keywords_tfief",keywords_tfief)
            keywords_jieba = getkeywords_jieba(text, stopwordsfile)
            # print("keywords_jieba",keywords_jieba)
            keywords_sentence_transformer = getkeywords_sentence_transformer(text)
            temp = [list(keywords_tfief.keys())]
            temp.append(keywords_jieba)
            temp.append([keywords_sentence_transformer])
    ...
    for i in range(df.shape[0]):
        if df.iloc[i,0] in dic and len(dic[df.iloc[i,0]])>1:
            df.loc[i,"tf-idf"] = ','.join(dic[df.iloc[i,0]][0])
            df.loc[i,"jieba"] = ','.join(dic[df.iloc[i,0]][1])
            df.loc[i,"bert"] = ','.join(dic[df.iloc[i,0]][2])
    df.to_csv(file_name.split('.')[0]+"res2.csv",index=0)
...
def getkeywords_key_bert(name):
    ...
    keywords = kw_extractor.extract_keywords(text, keyphrase_ngram_range=(1,1), stop_words=stopwords)
    print("Keywords of article", keywords)


if __name__ == '__main__':
    # parser = argparse.ArgumentParser()
    # parser.add_argument('--startDate', default="2008-1-1")
    # parser.add_argument('--endDate', default="2020-12-12")
    # parser.add_argument('--files', default="./openwall_content")
    ...
    files = "/home/chenqi/work/openwall/openwall_content"
    stopwordsfile = "/home/chenqi/work/openwall/stopwords.txt"
    # getkeywords_tfidf_sklearn("Hanno")
    # getkeywords_jieba("Eugene Teo", stopwordsfile)
    # getkeywords_rake_nltk("Eugene Teo")
    ...
    # keywords_to_scv("/home/chenqi/work/openwall/names.csv", stopwordsfile)
    # getemail("Kurt", "2008-10-10","2020-3-15")
    # date_list=getDateList(args.startDate, args.endDate)
    getkeywords_key_bert("Kurt")
